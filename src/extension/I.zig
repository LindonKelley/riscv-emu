...
pub const INSTRUCTIONS = struct {
    ...
    pub const FENCE = struct {
        ...
        pub fn execute(context: anytype, inst: inst_format.FENCE) void {
            const succ: FenceOperands = .{ .writes = inst.sw, .reads = inst.sr, .outputs = inst.so, .inputs = inst.si };
            const pred: FenceOperands = .{ .writes = inst.pw, .reads = inst.pr, .outputs = inst.po, .inputs = inst.pi };
